...
\begin{frame}\frametitle{Bidding}
    \begin{center}
        \begin{itemize}
...
\begin{frame}\frametitle{Bidding}
    \begin{center}
        Here is the general function for the algorithm
        \begin{equation*}
            f(x) = (round(Max(HeuristicSet)/20), Max(HeuristicSet).Suit)
        \end{equation*}
        \begin{equation*}
                \left[(Spades.Heuristic, Spades), (Hearts.Heuristic, Hearts),\\
                    (Clubs.Heuristic, Clubs) (Diamonds.Heuristic, Diamonds),\\
                    (Aces.Heuristic, Aces), (Pass.Heuristic, Pass)\right]
        \end{equation*}
    \end{center}
\end{frame}
